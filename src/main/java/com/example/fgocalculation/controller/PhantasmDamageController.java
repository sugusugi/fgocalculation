...
public class PhantasmDamageController {
    ...
    public String calculation(@Validated @ModelAttribute("servantParameter") PhantasmForm servantParameter,
            BindingResult bindingResult, @ModelAttribute("form") ServantSearchForm form, Model model) {
        ...
        double atk = sub.nullToZero(servantParameter.getAtk());
        double fou = sub.nullToZero(servantParameter.getFou());
        double spiritual = sub.nullToZero(servantParameter.getSpiritual());

        double atkBuf = 1 + sub.nullToZero(servantParameter.getAtkBuf()) / 100;
        double cardBuf = 1 + sub.nullToZero(servantParameter.getCardBuf()) / 100;
        double phantasmBuf = 1 + sub.nullToZero(servantParameter.getPhantasmBuf()) / 100;
        double specialBuf = sub.nullToZero(servantParameter.getSpecialBuf()) / 100;
        double phantasmSpecial = sub.nullToZero(servantParameter.getPhantasmSpecial()) / 100;

        double[] phantasmRates = { servant.getPhantasmRateLv1(), servant.getPhantasmRateLv2(),
                servant.getPhantasmRateLv3(), servant.getPhantasmRateLv4(), servant.getPhantasmRateLv5() };
        ...
        double answer = (atk + fou + spiritual) * 0.23 * classAtkRate * phantasmRate * (cardAtkRate * cardBuf) * atkBuf
                * (phantasmBuf + specialBuf) * phantasmSpecial;
        ...
        double answer1Max = answer1 * 1.099;
        double answer1Min = answer1 * 0.9;
        double answer2Max = answer2 * 1.099;
        double answer2Min = answer2 * 0.9;
        double answer3Max = answer3 * 1.099;
        double answer3Min = answer3 * 0.9;
